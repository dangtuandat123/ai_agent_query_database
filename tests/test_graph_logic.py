...
def test_graph_router_heuristic_avoids_false_positive_on_newest_word() -> None:
    ...
    result = agent.ask("Show newest trips in March 2018 from table_a")
...
def test_graph_followup_heuristic_handles_vietnamese_no_diacritics() -> None:
    tables = _tables()
    fake_db = FakeDB(tables=tables, rows=[{"id": 1}])
    ...
    first = agent.ask("Show one row from table_a")
    second = agent.ask("so sanh voi truoc cho table_b")
...
def test_graph_memory_stores_truncated_turn_values() -> None:
    tables = _tables()
    fake_db = FakeDB(tables=tables, rows=[{"id": 1}])
    fake_llm = FakeLLM(
        route="sql",
    ...
    _ = agent.ask("Q" * 2000, thread_id="thread-a")

    stored = agent._conversation_memory["thread-a"]
    assert len(stored["question"]) <= 500
    assert len(stored["sql_query"]) <= 2000
    assert len(stored["final_answer"]) <= 1200
